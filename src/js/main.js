...
var prediction = function($scope) {
  ...
  var countScores = function() {
    ...
    for (var i = 0; i < $scope.games.length; i++) {
      ...
      if ($scope.games[i].winner === "washington") {
        uwWins++;
      }
      else { otherWins++; }
    }

    $scope.huskies = uwWins;
  ...
  window.bam = function() {
    $scope.games.forEach(g => g.winner = "washington");
    $scope.$apply();
  }
  ...
  $scope.$watch(function() {
    ...
    if ($scope.games.every(game => game.winner)) {
      ...
      if (!alreadyComplete) {
        ...
        new Share(".share-results", {
          description: "I think the Huskies will go " + $scope.huskies + "-" + $scope.other + " this season. What’s your prediction for 2016? Make your picks: ",
          ui: {
            flyout: "bottom right",
    ...
    localStorage.setItem("huskies-prediction", JSON.stringify(filtered));
  });
